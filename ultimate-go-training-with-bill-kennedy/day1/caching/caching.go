...
// No real efficiency in this because not able to look at the cache line/TLB miss, but faster than column
// because allocators are same size/bulk of bytes are on the same OS pages because allocators are placing those data types
// together.
...
//Pretty much random memory access by having to look up a new location each time.
...
// Very fast because we are scanning through the data in order.
