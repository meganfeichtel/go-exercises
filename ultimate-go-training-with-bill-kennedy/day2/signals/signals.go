...
func boundedWorkPooling() {
	work := []string{"paper", "paper", "paper", "paper", "paper", 2000: "paper"} //don't do things like this - too clever
	...
	for _, wrk := range work { //start signalling the work to the goroutines, leveraging latency to our advantage
		ch <- wrk
	}
	close(ch)
	wg.Wait()
...
func cancellation() {
	...
	ch := make(chan string, 1) //this send can complete because there is 1 buffer space
	//ch := make(chan string) // this is a bug/goroutine leak! because there won't be a receiver to be able to get the send from the sender
